...
const canConfirm = computed(() => codeMatch.value && !!approvalDate.value && !busy.value);
...
async function confirm() {
  ...
  busy.value = true;
  emit('confirmed', { approvalDate: approvalDate.value });
}
</script>

<template>
  <UiModal
    title="품목허가 전이 — 보존기간 자동 연장 (단축 불가)"
    :model-value="modelValue"
    @update:model-value="close"
  >
    <UiAlert type="warning">
      ⚠️ 이 작업 이후 <strong>{{ project.projectCode }}</strong>에 연결된 모든 문서의 보존기간은
      <strong>{{ approvalDate || '(품목허가일 입력 필요)' }}부터 {{ project.retentionYears ?? '?' }}년</strong>으로
      자동 연장됩니다. 21 CFR Part 11 규제상 <strong>한번 연장된 보존기간은 줄일 수 없습니다.</strong>
    </UiAlert>

    <div class="ui-field">
      <label>품목허가일 <span aria-required="true">*</span></label>
      <input type="date" v-model="approvalDate" :max="today" required />
    </div>
